	...
	const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
		if ((isDraggingLeft || isDraggingRight) && rulerRef.current) {
			...
			if (container) {
				...
				if (isDraggingLeft) {
					const maxLeftPosition = PAGE_WIDTH - rightMargin - MIN_WIDTH;
					console.log(maxLeftPosition, ' maxLeftPosition')

					const newLeftPosition = Math.min(rawPosition, maxLeftPosition);
					console.log(newLeftPosition, ' newLeftPosition')

					setLeftMargin(newLeftPosition); // todo make collaborative
				} else if (isDraggingRight) {
					const maxRightPosition = PAGE_WIDTH - (leftMargin + MIN_WIDTH);
...
const Marker = ({
	...
	return (
		...
		>
			...
			/>
			<div
				className={'absolute left-1/2 top-4 transform -translate-x-1/4'}
				style={{
					height: '100vh',
					width: '1px',
